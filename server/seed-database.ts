...
// ✅ Prevent deep TypeScript recursion issues
...
// Generate synthetic furniture data via Gemini LLM
async function generateSyntheticData(): Promise<Item[]> {
  ...
  // ✅ Cast to Item[] to fix TypeScript unknown issue
  return parser.parse(response.content as string) as unknown as Item[]
}

// Create searchable summary from each furniture item
async function createItemSummary(item: Item): Promise<string> {
  return new Promise((resolve) => {
    ...
    const userReviews = item.user_reviews
      .map(r => `Rated ${r.rating} on ${r.review_date}: ${r.comment}`)
      .join(" ")
