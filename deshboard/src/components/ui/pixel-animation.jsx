...
export function PixelAnimation({ className = "", opacity = 0.15, speed = 1 }) {
  ...
  const getDelay = (x, y, width, height, direction) => {
    let dx = x - width * 0.5;
    let dy = y - height;
    if (direction) dy = y;
    ...
    for (let x = 0; x < width; x += gap) {
      for (let y = 0; y < height; y += gap) {
        ...
        animationRef.current.pixels.push(
          new Pixel(x, y, color, speedMultiplier, delay, delayHide, step, maxSize)
        );
      }
    }
  };
  ...
  const animate = () => {
    ...
    ctx.clearRect(0, 0, animationRef.current.width, animationRef.current.height);

    if (animationRef.current.ticker >= animationRef.current.maxTicker) animationRef.current.animationDirection = -1;
    else if (animationRef.current.ticker <= 0) animationRef.current.animationDirection = 1;

    let allHidden = true;

    pixels.forEach((pixel) => {
      if (animationRef.current.animationDirection > 0) pixel.show();
      else {
        pixel.hide();
    ...
    if (animationRef.current.animationDirection < 0 && allHidden) animationRef.current.ticker = 0;
  };
  ...
  const resize = () => {
    ...
    if (animationRef.current.request) cancelAnimationFrame(animationRef.current.request);
